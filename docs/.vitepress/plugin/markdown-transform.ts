...
import { REPO_BRANCH, REPO_PATH, docRoot, docsDirName, projRoot } from '../constants'
...
const transformComponentMarkdown = (
  ...
  if (isComponent && isHaveComponentStyle) { links.unshift([footerLocale.style, styleUrl]) }
...
const getExampleImports = (componentId: string) => {
  ...
  for (const item of files) {
    ...
    const name = camelize(`Bc-${componentId}-${file}`)
